...
//       console.log(response.headers["Accept-Encoding"]);
//     })
  ...
  async (req, res, next) => {
    try {
      ...
      if (updatedUser) {
        ...
        next(
          createError(404, `user with this id ${req.user._id} didn't found`)
        );
      }
    } catch (error) {
      next(error);
...
usersRouter.get(
  "/me/bookedRooms/:bookedRoomId",
  JWTAuthMiddleware,
  async (req, res, next) => {
    try {
      const user = await UsersModel.findById(req.user._id);
      if (user) {
        const writtenExp = user.roomsBooked.find(
          (aRoom) => req.params.bookedRoomId === aRoom._id.toString()
        );
        if (writtenExp) {
          res.send(writtenExp);
        } else {
          next(
            createError(
              404,
              `Experience with this id ${req.params.expId} didn't found`
            )
          );
        }
      } else {
...
usersRouter.put(
  "/:userId/bookedRooms/:bookedRoomId",
  JWTAuthMiddleware,
  adminOnlyMiddleware,
  async (req, res, next) => {
    try {
      const user = await UsersModel.findById(req.params.userId);
      if (user) {
        const index = user.roomsBooked.findIndex(
          (aRoom) => aRoom._id.toString() === req.params.bookedRoomId
        );
        if (index !== -1) {
          user.roomsBooked[index] = {
            ...user.roomsBooked[index].toObject(),
            ...req.body,
          };
          await user.save();
          res.send(user);
        ...
        next(
          createError(
            404,
            `User with this id ${req.params.userId} didn't found`
          )
        );
      }
    } catch (error) {
  ...
  async (req, res, next) => {
    try {
      ...
      if (user) {
        ...
        next(
          createError(404, `User with id ${req.params.userId} didn't found`)
        );
      }
    } catch (error) {
      next(error);
